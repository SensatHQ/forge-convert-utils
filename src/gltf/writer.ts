...
export class Writer {
  ...
  constructor(options: IWriterOptions = {}) {
    this.options = {
      maxBufferSize: isNullOrUndefined(options.maxBufferSize)
        ? MaxBufferSize
      ...
      filter:
        (options && options.filter) || ((dbid: number, fragid: number) => true),
    };
  ...
  protected createScene(imf: IMF.IScene): gltf.Scene {
    ...
    const manifestMaterials = this.manifest
      .materials as gltf.MaterialPbrMetallicRoughness[];
    ...
    if (
      ...
      if (up && front && distanceUnit) {
        ...
        if (left[0] * left[0] + left[1] * left[1] + left[2] * left[2] > 0.0) {
          ...
          console.warn(
            "Could not compute world matrix, leaving it as identity..."
          );
        }
      }
    }
  ...
  protected createMeshGeometry(
    geometry: IMF.IMeshGeometry,
    imf: IMF.IScene,
    outputUvs: boolean
  ): gltf.Mesh {
    let mesh: gltf.Mesh = {
    ...
    const indexBufferView = this.createBufferView(
    ...
    const positionBufferView = this.createBufferView(
    ...
    const positionAccessor = this.createAccessor(
      positionBufferViewID,
      5126,
      positionBufferView.byteLength / 4 / 3,
      "VEC3",
      positionBounds.min,
      positionBounds.max /*[fragmesh.min.x, fragmesh.min.y, fragmesh.min.z], [fragmesh.max.x, fragmesh.max.y, fragmesh.max.z]*/
    );
    ...
    if (normals) {
      const normalBufferView = this.createBufferView(
      ...
      const normalAccessor = this.createAccessor(
        normalBufferViewID,
        5126,
        normalBufferView.byteLength / 4 / 3,
        "VEC3"
      );
    ...
    if (colors) {
      const colorBufferView = this.createBufferView(
      ...
      const colorAccessor = this.createAccessor(
        colorBufferViewID,
        5126,
        colorBufferView.byteLength / 4 / 4,
        "VEC4"
      );
    ...
    if (geometry.getUvChannelCount() > 0 && outputUvs) {
      ...
      const uvBufferView = this.createBufferView(
  ...
  protected createLineGeometry(
    geometry: IMF.ILineGeometry,
    imf: IMF.IScene
  ): gltf.Mesh {
    let mesh: gltf.Mesh = {
      primitives: [],
    ...
    if (colors) {
      ...
      const colorAccessor = this.createAccessor(
        colorBufferViewID,
        5126,
        colorBufferView.byteLength / 4 / 3,
        "VEC3"
      );
      colorAccessorID = this.addAccessor(colorAccessor);
    }
  ...
  protected createPointGeometry(
    geometry: IMF.IPointGeometry,
    imf: IMF.IScene
  ): gltf.Mesh {
    let mesh: gltf.Mesh = {
      primitives: [],
  ...
  protected createMaterial(
    mat: IMF.Material | null,
    imf: IMF.IScene
  ): gltf.MaterialPbrMetallicRoughness {
  ...
  protected computeMeshHash(mesh: gltf.Mesh): string {
    return mesh.primitives
      .map((p) => {
        ...
        }/${p.attributes["NORMAL"] || ""}/${p.attributes["TEXCOORD_0"] || ""}/${
  ...
  protected computeBufferViewHash(bufferView: gltf.BufferView): string {
    return `${bufferView.buffer}/${bufferView.byteLength}/${
      bufferView.byteOffset || ""
    }/${bufferView.byteStride || ""}`;
  }

  protected computeAccessorHash(accessor: gltf.Accessor): string {
    return `${accessor.type}/${accessor.componentType}/${accessor.count}/${
      accessor.bufferView || "X"
    }`;
  }
  ...
  protected computeMaterialHash(
    material: IMF.IPhysicalMaterial | null
  ): string {
    if (!material) {
      return "null";
    }
