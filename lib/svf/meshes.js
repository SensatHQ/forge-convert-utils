 ...
 */
function* parseMeshes(buffer, logger) {
...
function parseMeshRAW(pfr) {
    ...
    const mesh = { vcount, tcount, uvcount, attrs, flags, comment, uvmaps, indices, vertices, min, max };
    if (normals) {
...
function parseLines(pfr, entryVersion, logger) {
    ...
    console.log(`LINE COLOR VALUE: ${pfr.getUint8()}`);
    const lines = {
        isLines: true,
        vcount: vertexCount,
        lcount: indexCount / 2,
...
function parsePoints(pfr, entryVersion) {
    console.assert(entryVersion >= 2);
    const vertexCount = pfr.getUint16();
    const indexCount = pfr.getUint16();
